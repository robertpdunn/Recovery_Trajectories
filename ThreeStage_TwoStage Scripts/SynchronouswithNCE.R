...
deltaUs= 0.2   #rate of kelp consumption by small urchins #0.2
deltaUm= 0.2   #rate  of kelp consumption by med. urchins #0.2
...
##Time series of biomass relative to pre-MPA equilibria N values
#pdf("Synchronous With NCE.pdf")
plot(out2$time,kelp, type="l", lwd=2,xlab="t (years since fishery closure)",
     ...
     main="Synchronous recovery: with NCE", bty="L",las=1)
...
maxfished<-max(y$FishedDens)  #9.747
fishedvol<-maxfished-y[200,9] #2.241
synchNCE<-y$FishedDens

maxcomm<-max(y$CommDens) #30.336
commvol<-maxcomm-y[200,8] #13.108
