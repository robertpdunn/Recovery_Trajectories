...
###----------parameters----------------------###
#state<-c(A=2545.075, Us=6.9208, Um=0.5640, Ul=0.0971, Ls=2.6556, L=9.6092)   #Unexploited equilibrium conditions no NCE
state<-c(A=2778.925, Us=6.450177, Um=0.6440198, Ul=0.1374821, Ls=2.626953, L=7.368650)   #Unexploited equilibrium conditions with NCE
...
#deltaUs= 0.2   #re-set rate of kelp consumption by small urchins due to fewer lobsters, ie NO non-consumptive effect (tmi)
#deltaUm= 0.2   #re-set rate  of kelp consumption by med. urchins due to fewer lobsters
...
#Calculate kelp volatility   #Don't use these
#maxkelp<-max(y$A) #kelp
#maxkelp   #kelp
#kelpvolit<-maxkelp-state[1]
#kelpvolit #106.578
...
fishedvol<-maxfished-y[400,9] #2.4276
preyNCE<-y$FishedDens
maxcomm<-max(y$CommDens) #30.749
commvol<-maxcomm-y[400,8] #13.522
...
##Try calculate return times using lsodar solver and a root function (deSolve paper in JStat: Soetaert et al. 2010, p. 8)
##Not working quite right yet
rootfun<-function(t, state, parms){
  dstate<-unlist(threestagetwostage(t, state, parms))
  sum(abs(dstate)) - .18
}
state<-c(A=1e3, Us=11, Um=2, Ul=1, Ls=5, L=15)   
out<-as.data.frame(lsodar(func=threestagetwostage, y = state, parms=parms, times=times, rootfun=rootfun))
tail(out,10)
#Now run scenario, extract the transient duration for each, then sum the total transient duration until back at unexploited eq.
