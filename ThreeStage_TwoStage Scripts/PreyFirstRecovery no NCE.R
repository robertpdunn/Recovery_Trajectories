...
setwd("~/PhD/Community Dyn. Model/MPA_Simulator Plots")
rm(list=ls())
require(deSolve)
...
#Now stop fishing for lobsters, don't include non-consumptive effect on urchin grazing.
...
y<-rbind(out2,out3)
y$time<-seq(1,400)
plot(y$time,y$A, ylim=c(0,3000), type="l", lwd=2, col="forestgreen") #Raw biomass time series
...
##Time series of abundance relative to pre-MPA equilibria N values
#pdf("PreyNoNCE.pdf")
plot(y$time,g[,1], type="l", lwd=2,xlab="Time (years since MPA)",
     ylab="Relative Abundance (Nt/N0)", col="forestgreen",ylim=c(0,2),
     main="Prey first recovery: no NCE", bty="L",las=1)
...
maxfished<-max(y$FishedDens) #11.762
fishedvol<-maxfished-y[400,9] #2.0556
preyNOnce<-y$FishedDens
maxcomm<-max(y$CommDens) #30.749
commvol<-maxcomm - y[400,8] #10.902

################
#RETURN TIME###
#Calculate manually by plotting and looking at y dataframe, 
#add up duration of first and 2nd transient periods until each is within 10% of equilibria



##################################
####Urchin Barren Initial State###   #These results identical to kelp forest state#
##################################
#Need to load model from top###
###----------parameters----------------------###
...
plot(y$time,g[,1], type="l", lwd=2,xlab="t (years since fishery closure)",
     ylab="Relative Abundance (Nt/N0)", col="forestgreen",ylim=c(0,2),
     main="Post-fishery closure dynamics: \n Prey Recovery no NCE", bty="L",las=1)
lines(y$time,g[,2], type="l", lwd=2, col="purple")
lines(y$time,g[,3], type="l", lwd=2, col="blue")
lines(y$time,g[,4], type="l", lwd=2, col="darkred")
lines(y$time,g[,5], type="l", lwd=2, col="red")
