...
FU=0.0         #close the lobster fishery
...
plot(y$time,g[,1], type="l", lwd=2,xlab="t (years since fishery closure)",
     ylab="Relative Abundance (Nt/N0)", col="forestgreen",ylim=c(0,2),
     main="Predator first recovery: no NCE", bty="L",las=1)
...
maxfished<-max(y$FishedDens)  #11.5
fishedvol<-maxfished-y[400,9] #1.794
predNOnce<-y$FishedDens
maxcomm<-max(y$CommDens) #30.3359
commvol<-maxcomm-y[400,8] #10.489
...
#######################################
###Recovery trajectory calculations####
####Starting from kelp forest state####
#######################################

#First need to add up all values in each compartment at each time step for a "community biomass" value (exclude kelp)
y$TotalBiomass<-rowSums(y[,c(5,7)])
plot(TotalBiomass~time, data=y, type="l")
###Calculating transient duration####

###Calculate volatility######
vol<-max(y$TotalBiomass)-y[400,7]
vol
